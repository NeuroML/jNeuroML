...
def main():
    ...
    for repo in all_repos:
        ...
        if mode == "update":
            ...
            if not op.isdir(local_dir):
                ...
                print(
                    "Creating a new directory: %s by cloning from GitHub" % (local_dir)
                )
            ...
            if switch_to_branch:
                if repo in dev_branch_repos:
                    if "jNeuroML" in repo and dont_switch_jneuroml_branch:
                        print(
                            "NOT switching repo %s to branch: %s"
            ...
            if not ("jNeuroML" in repo and dont_switch_jneuroml_branch):
    ...
    if mode == "update":
        ...
        if os.name != "nt":
            ...
            print("  ./jnml " + "../NeuroML2/LEMSexamples/LEMS_NML2_Ex2_Izh.xml")
        else:
            print("  jnml -validate " + "..\\NeuroML2\\examples\\NML2_FullNeuroML.nml")
            print("  jnml " + "..\\NeuroML2\\LEMSexamples\\LEMS_NML2_Ex2_Izh.xml")
    if mode == "clean":
...
def execute_command_in_dir(command, directory, exit_on_fail=True):
    ...
    if p.returncode != 0:
        ...
        print(
            return_str[0].decode("utf-8") if sys.version_info[0] >= 3 else return_str[0]
        )
        if exit_on_fail:
            exit(p.returncode)
